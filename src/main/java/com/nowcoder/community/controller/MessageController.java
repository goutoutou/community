...
//htmlescape方法将 HTML 特殊字符转义为 HTML 通用转义序列 htmlunescape实现反转换；
            String content = HtmlUtils.htmlUnescape(message.getContent());
            //JSONObject.parseObject()：将JSON字符串转化为实体类对象，传入参数为JSON字符串以及实体类的类型
            Map<String, Object> data = JSONObject.parseObject(content, HashMap.class);//字符串转为对象
    ...
    public String getNoticeDetail(@PathVariable("topic") String topic, Page page, Model model) {
        ...
        if (noticeList != null) {
            for (Message notice : noticeList) {
                ...
                map.put("postId", data.get("postId"));//关注那一类postid=null
                // 通知作者
