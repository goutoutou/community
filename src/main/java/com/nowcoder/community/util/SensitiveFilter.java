...
public class SensitiveFilter {
    ...
    private final TrieNode rootNode = new TrieNode();//初始化根节点

    /**
     * 初始化，读取敏感词
     */
    @PostConstruct//表示这是一个初始化方法，当容器实例化这个bean之后，这个方法将会自动调用
    public void init() {
        //读取txt文件中的字符
        try (InputStream inputStream = this.getClass().getClassLoader().getResourceAsStream("sensitive-words.txt");
             InputStreamReader inputStreamReader = new InputStreamReader(inputStream);//将字节流转成字符流
             BufferedReader bufferedReader = new BufferedReader(inputStreamReader);//缓冲流
        ) {
            String keyword;
            while ((keyword = bufferedReader.readLine()) != null) {
                this.addKeyWord(keyword);//将敏感词添加到前缀数中，txt文件中每个敏感词占一行
            }
    ...
    private void addKeyWord(String keyword) {
        ...
        for (int i = 0; i < chars.length; i++) {
            TrieNode childrenNode = tempNode.getChildrenNode(chars[i]);//查看数中是否存在这个节点
    ...
    public String filter(String text) {
        ...
        while (begin < text.length()) {//end<text.length更快
